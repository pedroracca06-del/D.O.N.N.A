...
4. ~~**Session Registry**~~ — **tool implemented locally** as `tools/cowork/session_registry.py` (81 tests). Detects worktree, branch, and scope collisions between concurrent sessions; live collisions block, stale or ambiguous ones escalate to you. It never deletes a record — `close` marks history. **The real registry at `${HOME}/.claude/nova-session-registry.json` has not been created**: every mutating operation stops until you approve initialization as a separate step. See [AUTOMATION_BACKLOG.md](AUTOMATION_BACKLOG.md#b14-session-registry--implemented-tool-only-registry-not-initialized).
5. **Worktree bootstrap validator** — verify a new worktree is correctly secured

All five are read-only in effect. Items 1-3 are done locally, item 4's tool is done locally with its registry still uninitialized, and item 5 is not started. Nothing beyond them is scheduled, and nothing here is pushed or merged.
